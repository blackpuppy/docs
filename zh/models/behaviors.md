...
模型的行为是一种组织CakePHP中定义的功能的方法。这允许我们分离并重用构成一种行为的逻辑, 并且这不需要继承。例如创建树形结构。行为提供了一种简单但强大的方式来增强模型, 允许我们通过定义一个简单的类变量为模型附加功能。模型就是这样去掉不属于它们建模所依据的商业逻辑的多余负担, 或者是不同模型都需要且可以推断出来的。
...
作为一个例子, 请考虑这样一个模型, 它让我们存取数据库中保存树形结构信息的一个表。删除, 添加, 和移动树的节点, 并不象删除, 插入和编辑表中的行那么简单。当把东西移来移去的时候, 很多记录要更新。与其为每个模型创建这些操作树的方法(对每个需要此功能的模型), 我们可以让我们的模型使用 :php:class:`TreeBehavior`, 或者用更正规的说法, 我们让模型具有树的行为。这就是把行为附加在模型上。就这么一行代码, 就可以让模型具有一整套新的方法, 可以和底层的结构交互。
...
CakePHP已有的行为包括树结构, 翻译内容, 权限控制列表的交互, 还有在 CakePHP Bakery (`http://bakery.cakephp.org
<http://bakery.cakephp.org>`_) 上的社区贡献的行为。本节将介绍为模型添加行为的基本使用模式, 如何使用 CakePHP 自带的行为, 以及如何创建你自己的行为。
...
在把行为附加到模型时, 某些行为要求或者允许定义一些设置。下面, 我们告诉 TreeBehavior 底层数据库表中"左节点"和"右节点"字段的名称::
...
我们也可以为一个模型附加多个行为。例如, 没有理由我们的 Category 模型只能有树结构的行为, 它也需要国际化支持::
...
就象我们能把行为从模型上完全分离, 我们也能附加新的行为。比如说, 我们熟悉的 Category 模型要有圣诞树的行为, 不过, 只是在圣诞节那天::
...
也有一个方法可以获得一个模型已附加的行为的列表。如果传入一个行为的名字, 它就会告诉我们那个行为是否已附加在模型上, 否则它就会给我们一个已附加行为的列表::
...
既然行为被所有使用它的模型实例共享, 好的做法是基于使用行为的别名/模型名来保存设置。当创建的行为的 ``setup()`` 方法被调用时::
...
模型的行为可以定义一些回调函数, 在模型同名的回调之前触发。行为的回调让行为可以抓住所附加的模型的事件, 并且改变参数, 或者加入额外的处理。
...
.. php:method:: setup(Model $Model, array $settings = array())
    ...
    当一个行为附加到模型时调用。设置来自模型的 ``$actsAs`` 属性。
...
.. php:method:: cleanup(Model $Model)
    ...
    当一个行为从模型分离时调用。基类方法根据 ``$model->alias`` 来删除设置。你可以重载,这个方法, 提供定制的清理功能。
...
.. php:method:: beforeFind(Model $Model, array $query)
    ...
    如果行为的 beforeFind 返回 false, find() 就会取消。返回一个数组就会增加用于 find 操作的查询参数。
...
.. php:method:: afterFind(Model $Model, mixed $results, boolean $primary)
    ...
    你可以用 afterFind 来增加 find 操作的结果。返回值会被交给链条中的下一个行为, 或者模型的 afterFind。
...
.. php:method:: beforeDelete(Model $Model, boolean $cascade = true)
    ...
    你可以从行为的 beforeDelete 返回 false, 来取消删除操作。返回 true 就允许它继续。
...
.. php:method:: beforeSave(Model $Model)
    ...
    你可以从行为的 beforeSave 返回 false, 来取消保存操作。返回 true 就允许它继续。
...
.. php:method:: beforeValidate(Model $Model)
    ...
    你可以用 beforeValidate 来改变模型的验证数组或者处理任何其它在验证之前的逻辑。从 beforeValidate 回调返回 false 会使验证中断并失败。
