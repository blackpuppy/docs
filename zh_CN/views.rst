...
CakePHP 的视图层是你和用户交流的方式。多数情况下，视图会显示 (X)HTML 文档
...
上面的视图文件可以用作父视图。它需要扩展它的视图来定义 ``sidebar`` 和 ``title`` 代码块。 ``content`` 代码块是个 CakePHP 创建的特殊代码块。它会包括所有子视图中未捕获的内容。假设我们的视图文件有一个 ``$posts`` 变量，含有与我们的帖子有关的数据。我们的视图可以像这样:
...
.. note::
    ...
    你应当避免使用 ``content`` 作为应用程序中代码块的名字。
    CakePHP 把它用于扩展视图中的未捕获内容。
...
.. note::
    ...
    你应当避免使用 ``content`` 作为代码块的名称。这被 CakePHP 内部用于扩展视图以及布局中的视图内容。
...
你可以用 ``fetch()`` 方法显示代码块。 ``fetch()`` 方法会安全地输出一个代码块，而在代码块不存在时返回''::
...
在2.3.0版本中，你可以为代码块提供当它不含任何内容时的缺省值。这让你可以容易地添加空状态时的缺省内容。你可以用第二个参数提供缺省值:
...
.. note::
    ...
    在视图文件中使用:php:meth:`HtmlHelper::css()`和:php:meth:`HtmlHelper::script()`时，对 'inline' 选项指定 'false'，就可以把 html 源代码放入同名的代码块中了。(关于用法的更多细节，请看 API。)
...
例如，如果我的网站的一个地方包含一小块广告栏(ad banner)，我也许会建一个新的含有小块广告位置的布局，并指定它为所有控制器动作的布局，象下面这样::
...
:php:meth:`View::element()`方法也支持元素的配置选项。支持的选项为 'cache' 和 'callbacks' 。例如::
    ...
    echo $this->element('helpbox', array(
        ...
        array(
            "cache" => "long_view", // 使用"long_view"缓存配置
            "callbacks" => true // 置为 true， 让元素的 before/afterRender 回调被调用
        )
    );

元素的缓存通过:php:class:`Cache`类来进行。你可以设置元素保存于任何你设置好的缓存配置。这给予你极大的灵活性，来决定元素保存在哪里，以及保存多长时间。要在一个应用程序中缓存同一元素的不同版本，用下面的格式提供一个唯一的缓存键值::
...
你可以用``requestAction()``方法来充分利用元素。``requestAction()``方法从控制器动作获取视图变量，再以数组返回。这允许你的元素以真正的 MVC 方式运作。创建一个控制器动作来为你的元素准备视图变量，然后在``element()``的第二个参数中调用``requestAction()``，就可以从控制器给元素提供视图变量。

为此，在 Post 的例子中，在你的控制器中添加下面这样的代码::
...
然后在元素中我们可以访问分页了的 posts 模型。要得到最近的五个帖子(posts)的有序队列，我们可以要这样做:
...
如果你提供一个缓存参数，你就可以充分利用 CakePHP 的视图缓存。如果设为 true，元素就会被缓存在'default'缓存配置中。否则，你可设置应该使用哪个缓存配置。更多配置 :php:class:`Cache` 的信息，请参看 :doc:`/core-libraries/caching`。下面是缓存元素的一个简单例子::

    echo $this->element('helpbox', array(), array('cache' => true));

如果你在同一个视图中多次渲染同样的元素，并启用了缓存，记得每次把'key'参数设置为不同名字。这样就可以防止后续的调用覆盖上一次元素调用的缓存结果。例如::
...
这样就会确保两次元素调用的结果被分别缓存。如果你要所有的元素缓存使用相同的缓存配置，你可以省去一些重复，只需把 :php:attr:`View::$elementCache` 设置为你要用的缓存配置。如果没有指定配置，CakePHP 就会使用这个配置。
...
要加载插件中的元素，使用 `plugin` 选项(已从1.x版本中的 `data` 选项移出)::
...
如果你使用插件，并且想在插件中使用元素，只需使用熟悉的 :term:`plugin syntax`。如果视图是为一个控制器/动作而渲染，插件名称会自动添加为所有使用的元素的前缀，除非已有了另一个插件名称。如果元素不在插件内，就会在主 APP 目录内查找。::

    echo $this->element('Contacts.helpbox');

如果你的视图是插件的一部分，你可以省略插件名称。例如，如果你在 Contacts 插件的 ``ContactsController`` 中::
...
你也许会需要创建定制的视图类，以用于新的数据视图，或者为你的应用程序增加额外的定制视图渲染逻辑。象 CakePHP 的大部分组成部件一样，视图类也有一些约定:

* 视图类应当放在``App/View``目录中。例如 ``App/View/PdfView.php``。
* 视图类应当以``View``为后缀。例如``PdfView``。
* 当引用视图类名称时，你应当省略``View``后缀。例如``$this->viewClass = 'Pdf';``。
...
视图方法在所有视图、元素和布局文件中都可使用。要调用任何视图方法，使用``$this->method()``。

.. php:method:: set(string $var, mixed $value)

    视图也有``set()``方法，就像控制器的``set()``方法一样。在你的视图文件中使用 set() 方法，会添加变量到以后渲染的布局和元素中。关于使用 set() 的更多信息，请看 :ref:`controller-methods`。
...
.. php:method:: getVars()
    ...
    获得当前渲染范围可以使用的视图变量列表，返回变量名称的数组。

.. php:method:: element(string $elementPath, array $data, array $options = array())

    渲染一个元素或部分视图。更多信息和例子，请看 :ref:`view-elements` 这一节。

.. php:method:: uuid(string $object, mixed $url)

    基于对象的类型和网址，给一个对象生成一个唯一的、非随机的 DOM ID。这个方法经常被象:php:class:`JsHelper`这样需要为元素生成唯一的 DOM ID 的助件使用::
...
.. php:method:: addScript(string $name, string $content)
    ...
    添加内容到内部的脚本缓冲区。这个缓冲区可在布局中用``$scripts_for_layout``来访问。这个方法在创建需要直接向布局添加 javascript 或者 css 的助件时，会有用。切记，从布局或布局中的元素添加的脚本，不会被加到``$scripts_for_layout``。这个方法最常用在助件内，比如 :doc:`/core-libraries/helpers/js` 和 :doc:`/core-libraries/helpers/html` 助件。
...
.. php:method:: startIfEmpty($name)
    ...
    只有当一个代码块为空时，才开始这个代码块。如果代码块已经定义，这个代码块(在这里定义的)的全部内容会被捕获并舍弃。
...
.. php:method:: fetch($name)
    ...
    取得代码块的值。对未定义的代码块会返回''。例子参见 :ref:`view-blocks` 一节。

    .. versionadded:: 2.1

.. php:method:: extend($name)

    用命名的视图/元素/布局扩展当前的视图/元素/布局。例子参见 :ref:`extending-views` 一节。
...
.. php:attr:: elementCache
    ...
    用来缓存元素的缓存配置。设置该属性会改变用来缓存元素的缺省配置。
    缺省的(缓存配置)可以在 element 方法中用 'cache'选项来改变。
...
.. php:attr:: output
    ...
    包含一个视图上一次的渲染内容，这或者是视图文件，或者是布局内容。

    .. deprecated:: 2.1
        请转而使用 ``$view->Blocks->get('content');``。
...
.. meta::
    :title lang=zh_CN: Views
    :keywords lang=zh_CN: view logic,csv file,response elements,code elements,default extension,json,flash object,remote application,twig,subclass,ajax,reply,soap,functionality,cakephp,audience,xml,mvc